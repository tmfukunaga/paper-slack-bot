...
既存Paper Watchとは独立して、Scopusで直近30日の候補を取得し、PlumX Metricsの
前日差分を優先して原則1日1本を同じSlackチャンネルへ投稿します。

- 実行時刻：毎日06:00 JST（GitHub Actionsでは`0 21 * * *` UTC）
- プログラム：`trending_watch/`
- 設定：`trending_watch.yaml`
- 状態：`data/trending-watch-state.json`
- Workflow：`.github/workflows/trending-organic-chemistry.yml`

追加するRepository Secretは`ELSEVIER_API_KEY`です。既存の
`OPENAI_API_KEY`、`SLACK_BOT_TOKEN`、`SLACK_CHANNEL_ID`も同じWorkflowで使用します。

初回実行は前日のPlumX値がないため現在値で順位付けし、2回目以降は日次増加を
優先します。PlumX指標は注目・利用の観測値であり、論文の品質評価ではありません。
Slack表示は既存Paper Watchと同じで、タイトルの先頭に`［話題］`を付けます。
内部のPlumXスコアやMatched keywordsはSlackへ表示しません。
...
### Trending Organic Chemistry

別ワークフロー`Trending Organic Chemistry`は、オンライン公開日が過去96時間以内の
有機合成関連論文をScopusから取得し、Mendeley `reader_count`の多い順に上位2報を
毎朝6時（Asia/Tokyo）に投稿します。同じ論文が連日上位の場合も再投稿します。

表示上のタイトル接頭辞は1位が`[Hot-1]`、2位が`[Hot-2]`です。Mendeley登録数そのものは
Slackには表示しません。
