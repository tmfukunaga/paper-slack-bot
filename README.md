...
投稿数は、Score 15以上を優先し、Score 11～14を1回3本・1日30本までの補充枠として扱います。基準は`config.yaml`だけで変更できます。
...
- **Score 15以上**：原則すべて投稿します。1回3本・1日30本の目安を超えても抑制しません。
- **Score 11～14**：補充枠です。その回の投稿成功数が3本未満、かつ当日の投稿成功数が30本未満の間だけ、スコア順に投稿します。
- **Score 10以下**：投稿しません。
- 高スコア論文の要約や投稿が失敗した場合、その論文は件数に数えません。空いた枠には次点のScore 11～14の論文を試します。
- 1日30本はScore 11～14にだけ適用する**ソフト上限**です。Score 15以上は常に優先されます。

例：

```text
24, 18, 15, 14, 13 → 24, 18, 15を投稿
14, 13, 12, 11, 10 → 14, 13, 12を投稿
20, 16, 14, 13 → 20, 16, 14を投稿
```
